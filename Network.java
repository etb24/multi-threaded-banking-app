	    ...
	    public void run() {
		    ...
		    while (true) {
		        ...
		        if (getClientConnectionStatus().equals("disconnected") && 
		            ...
		            // Both disconnected, can terminate the thread
		            System.out.println("\n DEBUG : Network.run() - Network thread terminating - both client and server disconnected");
		            setNetworkStatus("inactive");
		            return;
		        }
		        
		        // If either is still connected, yield CPU to other threads
		        Thread.yield();
		    }
		    
		    //System.out.println("\n DEBUG : Network.run() - network thread terminated");
		}
}
